...
export default function ShaderBackground() {
  useEffect(() => {
    // Create a div to hold the Three.js scene
    const embedRoot = document.createElement('div')
    embedRoot.className = "nt-embed"
    document.body.insertBefore(embedRoot, document.body.firstChild)
    ...
    importmap.textContent = JSON.stringify({
      imports: {
        ...
        "three": "https://cdn.skypack.dev/three@0.176.0",
        "react-three/fiber": "https://cdn.skypack.dev/@react-three/fiber@8.15.0"
      }
    })
    ...
    style.textContent = `
      .nt-embed {
        ...
        pointer-events: none;
        overflow: hidden;
        opacity: 0.6; /* Desktop opacity */
      }
      /* Higher opacity for mobile devices */
      @media (max-width: 768px) {
        .nt-embed {
          opacity: 0.75; /* Tablet opacity */
        }
      }
      @media (max-width: 480px) {
        .nt-embed {
          opacity: 0.95; /* Mobile opacity */
        }
      }
      .nt-embed canvas {
        width: 100%;
      ...
      /* Ensure content appears on top of the shader */
    ...
    // Use the embedRoot div we created earlier
    ...
    const TextureMesh = () => {
        ...
        // Add resize event listener to update values when screen size changes
        React.useEffect(() => {
            const handleResize = () => {
                if (!mesh.current) return;
                
                // Update uniforms based on screen size
                if (isMobileOrTablet()) {
                    mesh.current.material.uniforms.u_rays.value = 0.09
                    mesh.current.material.uniforms.u_reach.value = 0.85
                } else {
                    mesh.current.material.uniforms.u_rays.value = 0.05
                    mesh.current.material.uniforms.u_reach.value = 0.155
                }
            };
        ...
        useFrame(state => {
            ...
            if(mesh.current){
                ...
                // Dynamic adjustment for mobile and tablet
                if (isMobileOrTablet()) {
                    // Enhanced values for mobile/tablet
                    mesh.current.material.uniforms.u_rays.value = 0.09
                    mesh.current.material.uniforms.u_reach.value = 0.25
                } else {
        ...
        return React.createElement('mesh',
            ...
            React.createElement('planeGeometry',{args:[100,60]}), 
...
void main() {
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
                ...
                uniforms: {
                    u_colors: {value: [new THREE.Vector4(0.7098039215686275,0.9725490196078431,0.25882352941176473,1),new THREE.Vector4(0.3215686274509804,0.7450980392156863,1,1)]},
                    u_intensity: {value: 0.085},
                    u_rays: {value: 0.05}, // Original value for desktop
                    u_reach: {value: 0.155}, // Original value for desktop
                    u_time: {value: 0},
                    u_mouse: {value: [0,0]},
                    u_resolution: {value: [100,60]}
                },
                wireframe: false, 
    ...
    ), document.querySelector('.nt-embed'));
    `
    document.head.appendChild(script)

    // Cleanup function
    return () => {
      // Remove elements when component unmounts
      document.head.removeChild(importmap)
      document.head.removeChild(style)
      document.head.removeChild(script)
      const embedElement = document.querySelector('.nt-embed')
      if (embedElement) document.body.removeChild(embedElement)
    }
  }, [])
